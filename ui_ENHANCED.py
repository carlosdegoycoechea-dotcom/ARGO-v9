...
with st.sidebar:
    ...
    with st.expander("Settings"):
        ...
        st.divider()
        st.caption("Assistant Intelligence")
        prompt_mode = st.radio(
            "Response Mode",
            ["Advanced PMO Expert", "Simple Assistant"],
            index=0,
            help="Advanced mode provides expert-level PMO guidance with chain-of-thought reasoning, calculations, and industry standards. Simple mode provides basic Q&A."
        )
        # Map to internal values
        prompt_mode_value = "advanced" if prompt_mode == "Advanced PMO Expert" else "simple"
        ...
        with st.chat_message('assistant'):
            with st.spinner("Analyzing..."):
                try:
                    ...
                    # Get current project info from argo_state
                    current_project_name = argo_state.db.get_project_info(argo_state.current_project_id).get('name')
                    current_project_type = argo_state.db.get_project_info(argo_state.current_project_id).get('project_type')

                    # Determine if using advanced or simple mode based on settings
                    prompt_mode = search_settings.get('prompt_mode', 'advanced')
                    ...
                    system_prompt = get_system_prompt(
                        ...
                        include_library=search_settings.get('include_library', True)
                    )
