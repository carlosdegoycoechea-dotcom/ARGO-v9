...
class ModelRouter:
    ...
    def __init__(
        ...
        logger.info(
            f"ModelRouter inicializado con providers: {list(providers.keys())}, budget_monthly: {config.budget.get('monthly_usd', 0)} USD"
        )
    ...
    def route(
        ...
        try:
            logger.debug(
                f"Routing request - provider: {provider_name}, model: {model_name}, task_type: {task_type}, project_type: {project_type}"
            )
            ...
            logger.error(f"Error en route - provider: {provider_name}, model: {model_name}, error: {str(e)}")
    ...
    def _track_usage(
        ...
        try:
            ...
            logger.debug(
                f"Usage tracked - tokens: {total_tokens}, cost_usd: {round(cost, 4)}, provider: {response.provider}, model: {response.model}"
            )
    ...
    def _fallback_route(
        ...
        logger.info(
            f"Fallback activado - from_provider: {failed_provider}, to_provider: {fallback_provider}, model: {fallback_model}"
        )
